  ...
  export default {
    ...
    methods: {
      getList () {
        ...
        this.$http.get('http://127.0.0.1:8082/user/list', {
      ...
      createUser (formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            // 添加新用户
            this.$http.post('http://127.0.0.1:8082/user/addUser', {
              user: this.tempUser
            }, {
              headers: {
                'Authorization': this.$store.state.ShiroToken.token
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            if (this.tempUser.password === this.tempUser.confirmPassword) {
              this.$http.post('http://127.0.0.1:8082/user/updateUser', {
                user: this.tempUser
              }, {
                headers: {
                  'Authorization': this.$store.state.ShiroToken.token
      ...
      frozen (username, deleteStatus) {
        this.$http.post('http://127.0.0.1:8082/user/frozenUser', {
          username: username,
      ...
      removeUser (scope) {
        this.$confirm('此操作将删除用户: ' + scope.row.username + ', 是否继续?', '提示', {
          ...
          this.$http.post('http://127.0.0.1:8082/user/removeUser', {
