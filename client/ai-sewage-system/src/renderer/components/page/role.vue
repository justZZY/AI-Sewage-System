...
<script>
  export default {
    ...
    methods: {
      ...
      getAllPermisson () {
        // 查询所有权限
        this.$http.get('http://127.0.0.1:8082/user/listAllPermission').then(data => {
      ...
      getList () {
        ...
        this.$http.get('http://127.0.0.1:8082/user/listRole').then(data => {
      ...
      createRole () {
        ...
        // 添加新角色
        this.api({
          url: 'http://127.0.0.1:8082/user/addRole',
          method: 'post',
          data: this.tempRole
        }).then(() => {
          this.getList()
      ...
      updateRole () {
        ...
        // 修改角色
        this.$http.post('http://127.0.0.1:8082/user/updateRole', {
          data: this.tempRole
        }).then(() => {
          this.getList()
          this.dialogFormVisible = false
      ...
      removeRole ($index) {
        ...
        this.$confirm('确定删除此角色?', '提示', {
          ...
          _vue.api({
            url: 'http://127.0.0.1:8082/user/deleteRole',
            method: 'post',
            data: {
              roleId: role.roleId
            }
